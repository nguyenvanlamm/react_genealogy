...
const Facilities = () => {
  ...
  const [dataNewAddress, SetDataNewAddress] = useState<NewModel[]>([
    {
      id: 1,
      newsCategoryId: 1,
  ...
  const UpdateDataNewAddress = () => {
    const data: NewModel[] = [];
    news.forEach(item => {
      if (item.newsCategoryId == 4) {
        data.push(item);
      }
    });

    console.log("adasdasdasd: " + JSON.stringify(data));
    SetDataNewAddress(data);
  };
  ...
  const isLastSet =
    activeSlideIndex >= dataNewAddress.length - responsive[600].items;

  return (
    <div>
      <div className="utm text-[#9F302A] pb-[60px] text-[48px] leading-[48px] text-center font-normal">
        CÁC CƠ SỞ THỜ TỰ HỌ KIỀU
      </div>
      <AliceCarousel
        ...
        mouseTracking
        items={dataNewAddress.map((item, index) => (
          <div key={index} className="px-[15px]">
            <ItemCarousel id={item.id} imageUrl={item.imageUrl} content={item.content} title={item.title} />
          </div>
        ))}
        renderPrevButton={() => {
          return (
